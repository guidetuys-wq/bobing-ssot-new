...
export default function Sidebar() {
  ...
  const NavItem = ({ href, label, icon }) => (
    <Link href={href} className={`flex items-center gap-3 px-4 py-3 transition-all duration-200 mb-1 group ${isActive(href)}`}>
  ...
  const NavHeader = ({ title }) => (
    <div className="px-4 mt-6 mb-2 text-[10px] font-bold text-lumina-muted/50 uppercase tracking-widest font-display">
      {title}
    </div>
  );
  ...
  return (
    <aside className="hidden md:flex w-64 bg-lumina-surface flex-col h-full fixed md:static top-0 left-0 z-30 border-r border-lumina-border shrink-0 font-sans">
      {/* LOGO */}
      <div className="h-16 flex items-center px-6 border-b border-lumina-border shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-gradient-to-br from-lumina-gold to-[#F6C945] rounded-lg flex items-center justify-center text-black shadow-gold-glow">
      ...
      <nav className="flex-1 px-0 py-6 overflow-y-auto scrollbar-hide">
        ...
        <NavItem href="/customers" label="Pelanggan (CRM)" icon={<Icon d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0z" />} />
      ...
      <div className="p-4 border-t border-lumina-border bg-lumina-base/50 shrink-0">
        <button onClick={() => signOut(auth)} className="w-full py-2.5 text-xs font-bold text-rose-400 hover:text-rose-300 bg-rose-500/5 hover:bg-rose-500/10 rounded-lg border border-rose-500/20">
          Sign Out
        </button>
      </div>
    </aside>
