...
export default function Sidebar() {
  ...
  // Hapus semua definisi ikon SVG di sini
  ...
  return (
    <aside className={`bg-lumina-base border-r border-lumina-border font-sans z-40 hidden md:flex flex-col shrink-0 h-screen sticky top-0 transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
      ...
      {/* FOOTER SETTINGS - DIRENDER DARI footerNav */}
      <div className="p-4 border-t border-lumina-border bg-[#0F1115]">
        <Link href={footerNav.href} className={`flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-colors ${isSidebarCollapsed ? 'justify-center' : ''}`}>
           ...
           {!isSidebarCollapsed && (
             <div>
               <p className="text-xs font-bold text-white">{footerNav.label}</p>
