// app/(dashboard)/finance/accounts/page.js
"use client";
...
export default function FinanceAccountsPage() {
  ...
  const fetchAccounts = async (forceRefresh = false) => {
    ...
    try {
      ...
      const q = query(collection(db, 'chart_of_accounts'), orderBy('code', 'asc'), limit(300));
  ...
  const openEdit = (acc) => {
      setFormData({ id: acc.id, code: acc.code, name: acc.name, category: acc.category });
      setModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      invalidateCaches();
      fetchAccounts(true);
      toast.success('Akun berhasil disimpan!', { id: tId });
    } catch (error) {
  ...
  const deleteAccount = async (id) => {
    if (!confirm('Hapus akun ini? Pastikan saldo 0 agar tidak merusak Neraca.')) return;
    try {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50/80 text-[11px] font-bold text-text-secondary uppercase tracking-wider border-b border-border">
              <tr>
                <th className="pl-6 py-4 w-24">Code</th>
                <th className="py-4">Account Name</th>
                <th className="py-4">Category</th>
                <th className="py-4 text-right">Current Balance</th>
                <th className="text-center py-4 w-32">Status</th>
                <th className="text-right pr-6 py-4 w-24">Actions</th>
              </tr>
            </thead>
            <tbody className="text-sm divide-y divide-border/60">
              {loading ? (
                <tr><td colSpan="6" className="p-12 text-center text-text-secondary animate-pulse">Memuat data akun...</td></tr>
              ) : filteredAccounts.length === 0 ? (
                <tr><td colSpan="6" className="p-12 text-center text-text-secondary">Tidak ada data akun.</td></tr>
              ) : (
                    ...
                    <tr key={account.id} className="hover:bg-gray-50 transition-colors group">
                      ...
                      <td className="text-right pr-6 py-3">
                        <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={() => deleteAccount(account.id)} className="p-2 text-text-secondary hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <motion.div initial={{scale:0.95, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0.95, opacity:0}} className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-border">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div>
                  <label className="text-xs font-bold text-text-secondary uppercase mb-1 block">Account Code (Kode Akun)</label>
                  <input 
                    className="input-luxury font-mono text-lg font-bold tracking-wide" 
                    value={formData.code} 
