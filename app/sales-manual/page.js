...
export default function PosPage() {
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            setInvoiceData({ id: orderId, total, received, change: received - total }); setModalInvoiceOpen(true); setCart([]); setCashReceived('');
                    ...
                    {loading ? <div className="col-span-full text-center py-10 text-lumina-muted">Loading...</div> : filteredProducts.map(p => {
                        ...
                        return (
                            <div key={p.id} onClick={() => { setSelectedProdForVariant(p); setModalVariantOpen(true); }} 
                                ...
                                <div>
                                    <div className="flex justify-between mb-2"><span className="text-[10px] font-mono font-bold text-lumina-muted bg-lumina-base px-1.5 rounded">{p.base_sku}</span><span className={`text-[10px] px-2 rounded ${stock>0?'text-emerald-400 bg-emerald-900/30':'text-rose-400 bg-rose-900/30'}`}>{stock}</span></div>
                                    <h4 className="text-sm font-bold text-lumina-text group-hover:text-lumina-gold line-clamp-2">{p.name}</h4>
                                </div>
